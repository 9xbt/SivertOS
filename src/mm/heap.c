...
heap_pool* heap_create_pool(heap_ctrl* h, u64 initial_size) {
    ...
    if ((u64)pool == hhdm_offset) {
        serial_printf("heap_create_pool(): Couldn't allocate structure for the pool.\n");
        return NULL;
    }
    pool->next = pool; pool->prev = pool;
    pool->pool = pmm_alloc(initial_size / PAGE_SIZE);
    if (pool->pool == NULL) {
        serial_printf("heap_create_pool(): Couldn't allocate the pool.\n");
        return NULL;
    }
...
heap_ctrl* heap_create(u64 size) {
    ...
    if ((u64)h == hhdm_offset) {
        serial_printf("heap_init(): Couldn't allocate structure for a new heap.\n");
        return NULL;
    }
    h->pool_count = 0;
    ...
    if (h->pool_head == NULL) {
        serial_printf("heap_init(): Couldn't allocate heap's pool.\n");
        return NULL;
    }
    h->size = aligned_size * PAGE_SIZE;
    h->id = glob_id++;
    serial_printf("heap_init(): Created a new heap with size %ld at %lx.\n", aligned_size * PAGE_SIZE, (u64)h);
    return h;
}
...
void heap_free(heap_ctrl* h, void* ptr) {
    ...
    if (magic != HEAP_MAGIC) {
        serial_printf("heap_free(): Invalid magic at pointer %lx on heap %ld.\n", (u64)ptr, h->id);
        return;
    }
    heap_pool* p = h->pool_head;
    ...
    if (p->used)
        p->used = false;
    *(u64*)(ptr - 8) = 0; // Magic is set, but size is 0, the heap will understand that as free
    memset(ptr, 0, size);
}
...
void heap_resize(heap_ctrl* h, u64 new_size) {
    ...
    if (p == NULL) {
        serial_printf("heap_resize(): Couldn't allocate pool for resize of heap %ld.\n", h->id);
        return;
    }
    ...
    h->pool_head->prev = p;
    serial_printf("heap_resize(): Resized heap %ld successfully.\n", h->id);
}